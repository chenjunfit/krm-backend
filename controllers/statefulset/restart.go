...
func Restart(r *gin.Context) {
	info := models.Infor{}
	deployment := appV1.StatefulSet{}
	info.Item = &deployment
	kubeconfig := controllers.NewInfo(r, &info, "重启成功")
	var kubeUtilser kubeutils.KubeUtilser
	instance := kubeutils.NewStatefulSet(kubeconfig, &deployment)
	_, err := instance.Get(info.NameSpace, info.Name)
	if err != nil {
		logs.Error(map[string]interface{}{"资源类型": "StatefulSet", "msg": err.Error()}, "源数据查询失败")
